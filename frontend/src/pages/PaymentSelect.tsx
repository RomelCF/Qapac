...
export default function PaymentSelect() {
  ...
  function makeRandomQR() {
    const samples = [
      `https://api.qrserver.com/v1/create-qr-code/?size=220x220&data=${encodeURIComponent('QAPAC-' + Math.random().toString(36).slice(2))}`,
  ...
  async function confirmPaymentCash() {
    try {
      ...
      if (res.ok) {
        const now = new Date()
        let cliente: { nombres?: string; apellidos?: string; dni?: string } | undefined = undefined
        try {
          const cliRes = await fetch(`${API_BASE}/clientes/${p.idCliente}`)
          if (cliRes.ok) {
            const c = await cliRes.json(); cliente = { nombres: c.nombres, apellidos: c.apellidos, dni: c.dni }
          }
        } catch {}
        setSuccessData({
  ...
  function downloadReceipt() {
    ...
    const html = `<!doctype html><html><head><meta charset="utf-8"><title>Boleta - Qapac</title><style>${styles}</style></head><body>
      <div class="card">
        ...
        ${successData.cliente ? `<div class="row"><div class="muted">Cliente</div><div>${successData.cliente.nombres || ''} ${successData.cliente.apellidos || ''}</div></div>` : ''}
        ${successData.cliente?.dni ? `<div class="row"><div class="muted">DNI</div><div>${successData.cliente.dni}</div></div>` : ''}
